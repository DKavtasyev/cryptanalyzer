...
public class Conversation
{
	public static final String GREETINGS =
			...
			значением ключа. Для этого необходимо ввести флаг "%1$s".
			
			Так же она может раскодировать закодированный текст с известным ключом.
			Флаг для этой функции равен "%2$s"

			Так же она умеет безошибочно декодировать закодированный текст по шифру Цезаря с
			условием использования такого же алфавита. Для этого введи флаг "%3$s".
			...
			тогда не забудь указать флаг "%4$s".

			И если что, то ты вместо параметров в любой момент можешь написать команду help для вывода справки
			по структуре команд.""", Mode.ENCRYPT, Mode.DECRYPT, Mode.BRUTEFORCE_DECRYPT, Mode.STATISTICAL_DECRYPT);
	...
	public static final String[] CONVERSATION_ABOUT_DECRYPT_WITH_KEY = {
			"""
			Ты выбрал функцию дешифрования с известным ключом. Хорошо. Теперь нужно указать путь
			...
			String.format("Работа программы будет показана на файле по умолчанию %s из корневой папки проекта.", FilePaths.INPUT_DEFAULT_FILE_FOR_DECRYPT_WITH_KEY),

			"""
			Итак, существующий файл указан, теперь нужно указать криптографический ключ, используя который
			программа расшифрует текст из указанного файла.""",

			"Здесь без ключа не получится. Нужно ввести ключ для расшифрования.",

			String.format("Хорошо, параметры введены корректно. Зашифрованный файл будет находиться в корневой папке проекта под\n" +
					"названием %s.", FilePaths.OUTPUT_DEFAULT_FILE_FOR_DECRYPT_WITH_KEY),

			"""
			Хорошо, параметры введены корректно. Зашифрованный файл будет иметь тот же адрес и имя, но с пометкой
			_decrypted_with_key. Программа приступает к работе.
			"""
	};
	...
	public static final String HELP_TEXT =
			String.format("""
			[%1$s][Файл для шифрования][ключ]					команды для шифрования файлов.
			[%2$s][Файл для дешифрования][ключ]				команды для дешифрования файлов.
			[%3$s][Файл для дешифрования]						команды для дешифрования файлов методом брутфорс.
			[%4$s][Файл для дешифрования][Нормальный файл]	команды для статистического анализа файлов.
			[%5$s]							вывод алфавита шифрования (служебная информация).
			""", Mode.ENCRYPT, Mode.DECRYPT, Mode.BRUTEFORCE_DECRYPT, Mode.STATISTICAL_DECRYPT, Mode.PRINT_ALPHABET);
